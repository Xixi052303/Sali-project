...
- 无画面测试固定使用 `--headless`。每次运行先在 `tmp/` 创建互不复用的 `APPDATA`、`LOCALAPPDATA` 和日志路径，在同一个子进程环境中设置后再启动 Godot，避免访问正式 `user://logs`；不得只改 `--log-file` 而继续使用正式用户目录。
...
- 可视验证仅在画面事实无法由无窗口测试确认时执行，优先以命令行参数临时选择 Vulkan 与 Mobile；不得为验证改写 `project.godot`。专门验证 D3D12 时保留项目配置，并把本机已有 `dxgi.dll` 崩溃风险列入结果。
- `Failed to read the root certificate store` 在沙箱中可作为非阻断环境告警；只有退出状态、成功标志或新增脚本、资源、场景错误表明失败时才判失败。崩溃或卡死后保留 `tmp/` 日志、退出状态和复现条件，先判断用户目录、渲染后端和残留进程，再决定是否重试。
...
- 上述命令只表示参数基线；Codex 实际调用时还必须按验证流程注入本次运行独立的 `APPDATA`、`LOCALAPPDATA` 和 `tmp/` 日志路径。
