...
func configure(source_data: CustomerData, run_controller: RunController, index: int) -> void:
	data = source_data
	run = run_controller
	spawn_index = index
...
func _process(delta: float) -> void:
	...
	if run.is_world_scrolling():
		position.y += (run.world_scroll_speed + data.move_speed) * delta
...
func _draw() -> void:
	...
	draw_style_box(_body_style(data.body_color), body_rect)
